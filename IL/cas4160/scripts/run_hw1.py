"""
Runs behavioral cloning and DAgger for homework 1
...
from cas4160.infrastructure.bc_trainer import BCTrainer
from cas4160.agents.bc_agent import BCAgent
from cas4160.policies.loaded_gaussian_policy import LoadedGaussianPolicy
from cas4160.infrastructure.utils import MJ_ENV_KWARGS, MJ_ENV_NAMES


def run_bc(params):
    """
    Runs behavioral cloning with the specified parameters
...
def main():
    ...
    if args.do_dagger:
        ...
        assert args.n_iter > 1, (
            "DAgger needs more than 1 iteration (n_iter>1) \
            of training, to iteratively query the expert and train \
            (after 1st warmstarting from behavioral cloning)."
        )
    else:
        # Use this prefix when submitting. The auto-grader uses this prefix.
        logdir_prefix = "q1_"
        assert args.n_iter == 1, (
            "Vanilla behavioral cloning collects expert data \
            just once (n_iter=1)"
        )
